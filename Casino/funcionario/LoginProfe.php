...
<link rel="stylesheet" href="style.css">
   <head>
    ...
    <div class="main-block">
       <center><img src="logoA.png" width="200"></center>
     <?php error_reporting(0);?>
     <form action="" method="post">
     <hr>
     <center>Ingrese Rut y Contraseña de funcionario</center>
     <?php error_reporting(0);?>
     <form action="" method="post">
     <?php
            if ($_POST['btnIngreso']=="Ingresar") {
                    ...
                    $cnn = Conectar();
                    $rut = $_POST['txtUsuario'];
                    $pas = ($_POST['txtContra']);
                    $rs = mysqli_query($cnn, " select * from usuario where RUT='$rut' AND CONTRASEÑA = '$pas'");
                        if ($row = mysqli_num_rows($rs) != 0) {
                            if($row = mysqli_fetch_array($rs)){
                                ...
                                echo "<script type='text/javascript'>window.location='MenuFunc.php'</script>";            
                            }
                            }else{
